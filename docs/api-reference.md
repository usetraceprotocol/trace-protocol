...
Base URL: `https://api.trace-protocol.dev` (production) or `http://localhost:3100` (local)
...
Retrieve the verdict for a single Ethereum address.

**Path Parameters:**
- `address` (string, required): Ethereum public key (32-44 characters)
...
```json
{
  ...
  "message": "Ethereum address must be at least 32 characters"
}
```
...
**Request Fields:**
- `addresses` (string[], required): Array of Ethereum addresses (1 to 100)

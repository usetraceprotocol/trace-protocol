...
Trace Protocol is an AI agent verifier on Ethereum that classifies wallet addresses as
...
The scanner maintains persistent WebSocket connections to Ethereum RPC nodes and monitors
...
Data flow: Ethereum RPC -> WebSocket Listener -> Transaction Parser -> Signal Extractor
...
### 6. On-Chain Contract (`contracts/trace-attestation`)

An Foundry contract deployed on Ethereum that stores verdict attestations on-chain.
...
8. Optionally, verdict is written on-chain via trace-attestation
```
...
- Ethereum RPC node access (Alchemy recommended for production throughput)

...
const logger = pino({ name: "trace-api" });

const HOST = process.env.TRACE_API_HOST || "0.0.0.0";
const PORT = parseInt(process.env.TRACE_API_PORT || "3100", 10);
...
async function main(): Promise<void> {
  ...
  try {
    ...
    logger.info({ host: HOST, port: PORT }, "Trace Protocol API server started");
  } catch (err) {

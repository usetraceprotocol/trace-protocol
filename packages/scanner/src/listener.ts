...
const DEFAULT_CONFIG: ListenerConfig = {
  rpcUrl: "https://api.mainnet-beta.ethereum.com",
...
/**
 * EthereumListener maintains a WebSocket connection to a Ethereum RPC node
 ...
 */
export class EthereumListener extends EventEmitter<ScannerEvents> {
  ...
  private transactionBuffer: Map<string, EthereumTransactionMeta> = new Map();
  private flushTimer: ReturnType<typeof setInterval> | null = null;

  constructor(config: Partial<ListenerConfig> = {}) {
    super();
    this.config = { ...DEFAULT_CONFIG, ...config };
    this.logger = pino({ name: "trace-scanner-listener" });
  ...
  async stop(): Promise<void> {
    ...
    this.logger.info("Stopping Ethereum listener");
  ...
  async fetchHistoricalTransactions(
    ...
    const signatures = await this.connection.eth_getTransactionsByAddress(pubkey, {
      limit,
    });

    const transactions: EthereumTransactionMeta[] = [];
    ...
    for (const chunk of chunks) {
      ...
      for (let i = 0; i < parsed.length; i++) {
        const tx = parsed[i];
        if (tx && tx.meta) {
          transactions.push(this.parseEthereumTransaction(tx, sigs[i]));
        }
      }
    }
  ...
  private async handleLogNotification(signature: string): Promise<void> {
    ...
    const parsed = this.parseEthereumTransaction(tx, signature);
  ...
  /**
   * Convert a parsed Ethereum transaction into our internal representation.
   */
  private parseEthereumTransaction(
